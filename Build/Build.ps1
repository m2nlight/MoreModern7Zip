...
if (-not (Test-Path $vswhere)) {
    ...
    if (-not (Test-Path "$tempDir\VsWhere")) {
        ...
        $vswhere = "$tempDir\VsWhere\tools\vswhere.exe"
    }
}
...
if ($errorCode -ne 0) {
    $vsInstallPath = & "$vswhere" -prerelease -latest -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath
}
