...
if (Test-Path "$workDir\$BuildVersion.exe") {
    Rename-Item -Path "$workDir\$BuildVersion.exe" -NewName "$workDir\$BuildVersion-x64-YukiIsait.exe" -Force
    Write-Host "Successfully packaged $workDir\$BuildVersion-x64-YukiIsait.exe"
}